...
def build_toolset() -> list:
    """Assemble the agent's tools based on config (read_chart only if enabled)."""
    tools = [submit_decision, send_telegram]
    tools.insert(0, read_chart)   # always exposed; no-ops with a hint when disabled
    return tools
