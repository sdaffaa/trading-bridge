#!/usr/bin/env python3
"""H1 (1-hour) candlestick reel frame (1080x1920) for @liquidity.state.
Intraday XAUUSD OHLC is NOT available from the connected free data tools (all premium/denied),
so this is an EDUCATIONAL MODEL (clearly disclosed) with realistic gold prices and a proper
wick-based liquidity sweep. Feed a real H1/M15 OHLC array into OHLC[] to make it 100% real.
Same render(progress) animation as the daily version."""
import os, math

# ---- MODEL H1 close path (illustrative, realistic gold levels) ----
closes=[4108,4102,4097,4090,4084,4079,4074,4069,4063,4058,4054,4052,   # down into liquidity
        4051,4053,4052,                                                # consolidation (equal lows ~4050)
        4056,                                                          # 15: SWEEP candle (wick to 4041, close back up)
        4062,4069,4077,4085,4090,                                     # 16-20: rally, BOS above 4075
        4078,4066,4060,                                               # 21-23: pullback into OB
        4069,                                                         # 24: entry-confirmation bounce
        4079,4090,4100,4109,4118,                                     # 25-29: run to target
        4114,4110,4116,4112]                                          # 30-33: stall near target
N=len(closes)
LIQ=4050.0; SWEEP_LOW=4041.0; BOS=4075.0; ENTRY=4060.0; SL=4038.0; TP=4118.0
i_sweep=15; i_bos=18; i_entry=23; i_tp=29

# ---- build OHLC (open=prev close; wicks) with a real sweep wick on i_sweep ----
OHLC=[]
prev=4112.0
for i,c in enumerate(closes):
    o=prev; top=max(o,c); bot=min(o,c)
    uw=2+ (i*7)%5; lw=2+ (i*3)%5
    hi=top+uw; lo=bot-lw
    if i==i_sweep: lo=SWEEP_LOW; hi=top+2         # deep sweep wick, small upper
    if i==i_bos:   hi=max(hi,c+3)                 # clean break
    if i==i_tp:    hi=max(hi,TP+3)
    OHLC.append((o,hi,lo,c)); prev=c

# hourly time labels
def tlabel(i):
    h=(9+i)%24; return f"{h:02d}:00"

# ---- transforms ----
X0,X1=110,970; Y0,Y1=760,1500
pmax=max(h for _,h,_,_ in OHLC); pmin=min(l for _,_,l,_ in OHLC)
PMAX=pmax+6; PMIN=pmin-6
def X(i): return X0+(i+0.5)/N*(X1-X0)
def Y(p): return Y0+(PMAX-p)/(PMAX-PMIN)*(Y1-Y0)
CW=(X1-X0)/N*0.56

svg=[]
lo_grid=int(math.ceil(PMIN/10)*10)
for pr in range(lo_grid,int(PMAX),10):
...
for i in range(0,N,6):
    svg.append(f'<text x="{X(i):.0f}" y="{Y1+40:.0f}" fill="#93A2A8" font-size="20" font-family="Tajawal" text-anchor="middle">{tlabel(i)}</text>')

def el(inner,t0,cls="mk"): return f'<g class="{cls}" data-t0="{t0:.3f}">{inner}</g>'
for i,(o,h,l,c) in enumerate(OHLC):
    up=c>=o; col="#2E8CA6" if up else "#122F3E"
    x=X(i); bt=Y(max(o,c)); bb=Y(min(o,c)); hh=max(3,bb-bt)
    inner=(f'<line x1="{x:.1f}" y1="{Y(h):.1f}" x2="{x:.1f}" y2="{Y(l):.1f}" stroke="{col}" stroke-width="2.4"/>'
           f'<rect x="{x-CW:.1f}" y="{bt:.1f}" width="{2*CW:.1f}" height="{hh:.1f}" fill="{col}"/>')
    t0=0.03+0.47*(i/(N-1))
    svg.append(f'<g class="cndl" data-t0="{t0:.3f}" style="transform-box:fill-box;transform-origin:{x:.1f}px {bb:.1f}px;">{inner}</g>')
...
def hline(p,color,dash="6 7",t0=0.55):
    ...
    return el(s,t0)
svg.append(hline(LIQ,"#122F3E",t0=0.55))
svg.append(hline(BOS,"#2E7D96",t0=0.70))
svg.append(el(f'<circle cx="{X(i_sweep):.1f}" cy="{Y(SWEEP_LOW):.1f}" r="10" fill="#D24B4B"/>',0.60))
ax=X(i_bos); ay1=Y(BOS)
svg.append(el(f'<line x1="{ax:.1f}" y1="{Y(OHLC[i_bos][3])+26:.1f}" x2="{ax:.1f}" y2="{ay1:.1f}" stroke="#2E7D96" stroke-width="3"/>'
...
bx0=X(i_entry-1)-CW-6; bx1=X(i_sweep)+CW+6
svg.append(el(f'<rect x="{bx1:.1f}" y="{Y(ENTRY+3):.1f}" width="{bx0-bx1:.1f}" height="{Y(LIQ-2)-Y(ENTRY+3):.1f}" fill="#2E7D96" fill-opacity="0.16" stroke="#1E627A" stroke-width="1"/>',0.78))
lx=X(i_entry)-CW
...
L=[
 lbl(X1-6,Y(LIQ)-48,"سيولة سابقة","#122F3E",26,"end",0.57),
 lbl(X(i_sweep)+16,Y(SWEEP_LOW)+8,"سحب سيولة","#D24B4B",28,"start",0.62),
 lbl(X0+8,Y(BOS)-44,"تغيّر نمط","#2E7D96",28,"start",0.72),
 lbl(X(i_bos)-14,Y(OHLC[i_bos][3])-8,"كسر","#2E7D96",24,"end",0.76),
 lbl(X(i_entry)+16,Y(ENTRY)-6,"منطقة الدخول","#1E627A",24,"start",0.80),
 lbl(X1-6,Y(TP)-42,"هدف  4118","#1E627A",28,"end",0.86),
 lbl(X1-6,Y(SL)+8,"ستوب  4038","#D24B4B",26,"end",0.86),
]
LABELS="".join(L)
...
def su(a,b):return(a[0]-b[0],a[1]-b[1],a[2]-b[2])
...
def rtt(ax,an):
    x,y,z=nr(ax);c=math.cos(an);s=math.sin(an);t=1-c
    return((t*x*x+c,t*x*y-s*z,t*x*z+s*y),(t*x*y+s*z,t*y*y+c,t*y*z-s*x),(t*x*z-s*y,t*y*z+s*x,t*z*z+c))
def al(a,b):
    a=nr(a);b=nr(b);v=cr(a,b);c=dt(a,b)
    if c>0.9999:return((1,0,0),(0,1,0),(0,0,1))
    if c<-0.9999:return rtt((1,0,0),math.pi)
    return rtt(v,math.acos(max(-1,min(1,c))))
R=al(nr(ad(ad(Vv[0],Vv[11]),Vv[5])),(0,0,1));P=[mv(R,v)for v in Vv]
...
def pj(p):return(120+p[0]*100,120-p[1]*100)
LIv=nr((-0.32,0.75,0.58))
def sil(t):
...
ff=[]
for tri in FA:
    a,b,c=(P[i]for i in tri);nn=nr(cr(su(b,a),su(c,a)))
    if nn[2]<=0.03:continue
    ff.append(((a[2]+b[2]+c[2])/3,tri,max(0,dt(nn,LIv))))
ff.sort(key=lambda x:x[0]);pth=[]
for _,tri,sh in ff:
    d="M "+" L ".join(f"{pj(P[i])[0]:.1f},{pj(P[i])[1]:.1f}" for i in tri)+" Z"
...
reward=TP-ENTRY; RR=reward/(ENTRY-SL)
HTML=f"""<!DOCTYPE html>
<html lang="ar" dir="rtl"><head><meta charset="UTF-8"/>
<title>Liquidity State — H1 Candlestick Reel</title>
...
.result .tick{{font-size:38px;color:#2E8CA6;font-weight:900;}} .result .t{{font-size:38px;font-weight:800;color:#0F2E3C;}}
...
<div class="slide" data-canvas-width="1080" data-canvas-height="1920">
  ...
  <div class="stage" id="stage">
    ...
    {LABELS}
    <div class="result" id="res"><span class="tick">✓</span><span class="t">وصل الهدف</span><span class="v">+${reward:.0f} · RR 1:{RR:.1f}</span></div>
    <div class="cta" id="cta">للملف كامل اكتب <b>بلوك</b> بالتعليقات</div>
  </div>
  <div class="foot">نموذج تعليمي توضيحي — فريم H1 (ساعة) · أسعار توضيحية ليست بيانات سوق حقيقية · لغرض تعليمي</div>
</div>
<script>
function ease(x){{return 1-Math.pow(1-x,3);}}
function back(x){{const c=1.70158;return 1+(c+1)*Math.pow(x-1,3)+c*Math.pow(x-1,2);}}
function render(p){{
  document.querySelectorAll('.cndl').forEach(g=>{{const t0=+g.dataset.t0,d=0.06;let a=(p-t0)/d;a=Math.max(0,Math.min(1,a));
    g.style.opacity=a>0?1:0;const s=a>=1?1:back(a);g.style.transform='scaleY('+s.toFixed(3)+')';}});
  document.querySelectorAll('.mk,.lbl').forEach(g=>{{const t0=+g.dataset.t0,d=0.05;let a=(p-t0)/d;a=Math.max(0,Math.min(1,a));g.style.opacity=ease(a);}});
  ...
  document.getElementById('res').style.opacity=Math.max(0,Math.min(1,(p-0.93)/0.05));
  document.getElementById('cta').style.opacity=Math.max(0,Math.min(1,(p-0.95)/0.05));
}}
function fromHash(){{const m=/p=([0-9.]+)/.exec(location.hash);render(m?parseFloat(m[1]):1);}}
window.addEventListener('hashchange',fromHash);fromHash();
</script>
</body></html>"""
out=os.path.join(os.path.dirname(__file__),"reel_frame_h1.html")
open(out,"w",encoding="utf-8").write(HTML)
print("wrote",out,f"| N={N} entry={ENTRY} SL={SL} TP={TP} RR={RR:.2f} reward=${reward:.0f}")
