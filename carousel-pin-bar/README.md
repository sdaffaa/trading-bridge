...
| 6 | `06-examples.png` | أمثلة فاشلة حقيقية من الذهب (بيانات فعلية، مرسومة بستايل البراند) |
...
## بيانات الشارت (حقيقية)
شارت الصفحة ٦ مبني على شموع ذهب يومية حقيقية (GCUSD، يطابق XAUUSD) مسحوبة عبر
FMP، ونماذج الـ Pin Bar الفاشلة مكتشفة برمجياً — لا شموع مخترعة:
- `data/gc_daily.json` — شموع الذهب اليومية (OHLC).
- `detect_pinbars.py` — كاشف نماذج Pin Bar التي فشلت (قمة/قاع كاذب) مع قياس حركة الفشل.
- الأمثلة المختارة: قمة كاذبة **2025-12-12** (واصل صعود +167$)، قاع كاذب **2026-05-28** (واصل هبوط −136$).
