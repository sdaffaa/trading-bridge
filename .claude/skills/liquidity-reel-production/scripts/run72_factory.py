# -*- coding: utf-8 -*-
"""تشغيلة ٧٢ — مصنع المحتوى اليومي (§11): ثلاثةُ كاروسيلاتٍ وريلان وخمسةُ أدلة.

مفاتيحُها `content/run72_*.json`، وقيودُها بوسم `run72-<الوحدة>`،
ومخرجاتُها `car72_*` و`guide72_*`، والفرزُ يُقرأ من `X72.SYN` لا من
`X72.SETS`.

ووحداتُ اليوم ثلاثةُ أصناف: اثنتان على سوقٍ حقيقي (`register_real`)،
وواحدةٌ محاكاةٌ ببذورٍ معلنة (`register_synthetic`)، واثنتان عدٌّ يشمل
النوافذ الستّ فلا ترسمان سلسلةَ أسعارٍ أصلاً — ولا شيءَ فيهما يُحجز في
سجلّ عدم التكرار، فتُستثنيان من الحجز والتسجيل معاً (`X72.COUNT`).

    python3 run72_factory.py [slug ...]
"""
import json, os, sys
...
import run72_charts as X72

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run72-factory-2026-09-17"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run72_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run72_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run72_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car72_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide72_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run72-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run72_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X72.SETS:
        if slug in X72.REAL:
            assert X72.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X72.unit_charts(slug)
...
def _claim(slug, r):
    ...
    ووحدةُ العدّ لا سلسلةَ لها أصلاً — لا نافذةً ولا بذرة — فلا تُحجز:
    الحجزُ يمنع تكرارَ شارتٍ، وهي لا شارتَ فيها."""
    if slug in X72.COUNT:
        return
    if slug in X72.SYN:
        seed, anch = X72.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run72-{slug}")
    else:
        RC.claim_fresh(r, f"run72-{slug}")
...
def _register(slug, r):
    ...
    if slug in X72.SYN:
        seed, anch = X72.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run72-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run72-{slug}")
