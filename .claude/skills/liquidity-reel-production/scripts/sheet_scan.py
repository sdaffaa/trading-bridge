...
PER_CLS = 40          # نوافذ لكل فئة — سقف كمّي لا عتبة جودة
# رُفع من ١٠ إلى ٤٠: العشرة الأولى في كل فئة صارت كلها منشورة، فمسحٌ
# جديد يعود بنافذة واحدة حرّة وهو يملك المئات. الشروط كما هي — ≥٤ شمعات
# بعد الدخول، وبلوغ ٢R داخل النافذة، ومخاطرة ≥٦ نقاط أساس — ولا واحد
# منها مسّه الرفع. المتغيّر عددُ ما يُعرض عليها فقط.
...
# مصادر المسح — **قائمة تُوسَّع ولا تُخفَّض عتبة**. حين يعود المسح بلا
# نافذة حرّة يُسأل أولاً: أنفد ما يُعرض عليه أم ما يقبله؟ الأول يُعالَج
# بأدواتٍ وفريماتٍ أكثر. (وُسّعت 2026-09-04 داخل الصندوق فضاعت من
# المستودع؛ وأُثبتت هنا 2026-09-06 كي تعيش عبر التشغيلات.)
# و`PER_CLS` و`MAXW` و`MIN_AFTER` و`MIN_RISK_BP` كما هي حرفاً.
_TF = {"15m": 900, "30m": 1800, "1h": 3600, "1d": 86400}
# وُسّعت 2026-09-13: المسحُ عاد ٣٥ نافذة يومين متتاليين، ٣٤ منها مقيَّدة
# في السجل — فالنافد هو **المعروض** لا المقبول، وعلاجُه بالقائمة لا بالعتبة.
# والعملاتُ الرقمية تتداول في العطلة، فهي وحدها تُنبت نوافذ يوم أحدٍ مغلق.
SRC = [(sym, tf, _TF[tf]) for sym, tfs in [
    ("GC=F",     ("15m", "30m", "1h")),          # الذهب
    ("SI=F",     ("15m", "30m", "1h")),           # الفضة
    ("PL=F",     ("30m", "1h")),                  # البلاتين
    ("HG=F",     ("30m", "1h")),                  # النحاس
    ("CL=F",     ("15m", "30m", "1h")),           # الخام
    ("BZ=F",     ("1h",)),                        # برنت
    ("HO=F",     ("1h",)),                        # زيت التدفئة
    ("NG=F",     ("30m", "1h", "1d")),            # الغاز
    ("RB=F",     ("1h", "1d")),                   # البنزين
    ("KC=F",     ("1d",)),                        # البنّ
    ("ZC=F",     ("1h", "1d")),                   # الذرة
    ("ZS=F",     ("1h", "1d")),                   # الصويا
    ("ZW=F",     ("1d",)),                        # القمح
    ("YM=F",     ("15m", "30m", "1h", "1d")),     # الداو
    ("NQ=F",     ("15m", "30m", "1h", "1d")),     # الناسداك
    ("ES=F",     ("15m", "30m", "1h", "1d")),     # الإس آند بي
    ("RTY=F",    ("1h", "1d")),                   # راسل
    ("ZN=F",     ("1h", "1d")),                   # سندات ١٠ سنوات
    ("6A=F",     ("1h", "1d")), ("6B=F", ("15m", "30m", "1h")),
    ("6E=F",     ("1h", "1d")), ("6C=F", ("1h", "1d")),
    ("6J=F",     ("1h", "1d")), ("6S=F", ("1d",)),
    ("EURUSD=X", ("15m", "30m", "1h")),
    ("GBPUSD=X", ("15m", "30m", "1h")),
    ("USDJPY=X", ("15m", "30m", "1h")),
    ("AUDUSD=X", ("15m", "30m", "1h", "1d")),
    ("USDCAD=X", ("1h",)), ("USDCHF=X", ("1h",)),
    ("EURJPY=X", ("1h",)), ("GBPJPY=X", ("1h",)),
    ("EURGBP=X", ("1h",)),
    ("NZDUSD=X", ("1h",)), ("AUDJPY=X", ("1h", "1d")),
    # العملات الرقمية — سوقٌ لا يغلق، فهي مصدرُ النوافذ في العطلات
    ("BTC-USD",  ("15m", "30m", "1h")),
    ("ETH-USD",  ("15m", "30m", "1h", "1d")),
    ("SOL-USD",  ("30m", "1h")),
    ("XRP-USD",  ("30m", "1h")),
    ("BNB-USD",  ("1h",)), ("ADA-USD", ("1h",)),
    ("DOGE-USD", ("1h",)), ("LINK-USD", ("1h",)),
    ("AVAX-USD", ("1h",)), ("DOT-USD", ("1h",)),
    ("LTC-USD",  ("1h", "1d")),
    ("SPY",      ("1d",)), ("IWM", ("1d",)), ("SLV", ("1d",)),
    ("QQQ",      ("1d",)), ("GLD", ("1d",)), ("TLT", ("1d",)),
    ("USO",      ("1d",)), ("XLE", ("1d",)), ("EEM", ("1d",)),
] for tf in tfs]
allc = {}; series = {}
