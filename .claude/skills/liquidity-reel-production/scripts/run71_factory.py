# -*- coding: utf-8 -*-
"""تشغيلة ٧١ — مصنع المحتوى اليومي (§11): ثلاثةُ كاروسيلاتٍ وريلان وخمسةُ أدلة.

مفاتيحُها `content/run71_*.json`، وقيودُها بوسم `run71-<الوحدة>`،
ومخرجاتُها `car71_*` و`guide71_*`، والفرزُ يُقرأ من `X71.SYN` لا من
`X71.SETS`.

والمخزون كان قد نفد صباح اليوم، فأُعيد ملؤه بمسحٍ جديد (٥١ مرشّحاً، ٢٨
حرّةٌ بفحص السجل، أُدخلت ستٌّ المخزونَ بالفهارس ٧٢–٧٧). فثلاثُ وحداتٍ على
سوقٍ حقيقي (`register_real`) واثنتان سلسلتان مولّدتان
(`register_synthetic`) بقاعدة §11 البديلة.

    python3 run71_factory.py [slug ...]
"""
import json, os, sys
...
import run71_charts as X71

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run71-factory-2026-09-16"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run71_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run71_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run71_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car71_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide71_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run71-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run71_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X71.SETS:
        if slug in X71.REAL:
            assert X71.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X71.unit_charts(slug)
...
def _claim(slug, r):
    ...
    if slug in X71.SYN:
        seed, anch = X71.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run71-{slug}")
    else:
        RC.claim_fresh(r, f"run71-{slug}")


def _register(slug, r):
    if slug in X71.SYN:
        seed, anch = X71.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run71-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run71-{slug}")
