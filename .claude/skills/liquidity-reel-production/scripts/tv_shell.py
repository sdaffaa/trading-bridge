...
def tf_row(exec_tf):
    """صفّ الفريمات وأوّله فريم التنفيذ الفعلي.

    الصفّ كان ثابتاً يبدأ بـ«٥د»، فنافذة الثلاث دقائق كانت تُضيء شريحة
    الخمس — والمنصّة تكذب على المشاهد بفريمٍ غير الذي رُسمت عليه الشموع.
    فريمُ التنفيذ يتصدّر الصفّ دائماً، وما بعده الأعلى منه فقط."""
    # الصفّ ستّ شرائح. إن لم يكن فريم التنفيذ منها زُوحم أدناها (لا أعلاها):
    # الفريمات العليا يحتاجها التحليل نفسه — «١د» و«٤س» و«يومي» لوحاتٌ في
    # الريل، وإسقاط أحدها يترك نقرةً على شريحة غير موجودة.
    rest = [k for k in TF_ORDER if k != exec_tf]
    return [exec_tf] + rest[-5:]
