# -*- coding: utf-8 -*-
"""وحدة «خط الاتجاه» — كاروسيل 8 صفحات + دليل PDF. نفس معمار run15."""
import json, os
...
import run19_charts as RC
import chart_registry

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.join(HERE, "..", "content")
TOTAL = 8
RUN_ID = "run19-trend-2026-08-04"
KW, GTITLE, GEYEB = "ترند", "خط الاتجاه<br>رسماً واختباراً", "دليل — برايس أكشن"
...
def build():
    C = json.load(open(os.path.join(CONT, "run19_trend.json"), encoding="utf-8"))
    ...
    build_carousel(slides, f"{KW} — Liquidity State",
                   os.path.join(HERE, "car19_trend.html"), extra_css=X19)
    ...
    n = build_guide(cfg, os.path.join(HERE, "guide19_trend.html"))
    print(f"car19: {len(slides)} slides | guide19: {n} pages | charts: {tk}")
