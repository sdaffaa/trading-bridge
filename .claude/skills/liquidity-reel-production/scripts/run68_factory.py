# -*- coding: utf-8 -*-
"""تشغيلة ٦٨ — مصنع المحتوى اليومي (§11): ثلاثة كاروسيلات وريلان وخمسة أدلة.

مفاتيحُها `content/run68_*.json`، وقيودُها بوسم `run68-<الوحدة>`،
ومخرجاتُها `car68_*` و`guide68_*`، والفرزُ يُقرأ من `X68.SYN` لا من
`X68.SETS`.
...
RUN_ID = "run68-factory-2026-09-13"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run68_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run68_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run68_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car68_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide68_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run68-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run68_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X68.SETS:
        if slug in X68.REAL:
            assert X68.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X68.unit_charts(slug)
...
def _claim(slug, r):
    ...
    if slug in X68.SYN:
        seed, anch = X68.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run68-{slug}")
    else:
        RC.claim_fresh(r, f"run68-{slug}")


def _register(slug, r):
    if slug in X68.SYN:
        seed, anch = X68.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run68-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run68-{slug}")
