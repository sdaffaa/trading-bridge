# -*- coding: utf-8 -*-
"""تشغيلة ٦٨ — الرندر والتغليف: صور الكاروسيل ١٠٨٠×١٣٥٠ وأدلة PDF بلا فقد.
...
OUT = os.path.join(HERE, "out68")


def units():
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if fn.startswith("run68_") and fn.endswith(".json"):
...
def main(slugs=None):
    ...
    for slug in (slugs or list(U)):
        ...
        if C.get("media") != "reel":
            ...
            car_render.render(os.path.join(HERE, f"car68_{slug}.html"), d)
        ...
        car_render.render(os.path.join(HERE, f"guide68_{slug}.html"), g)
