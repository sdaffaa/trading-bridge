...
def _y_exec(y):
    ...
    # فوق القمم دائماً. وكان هنا حدٌّ سفليّ عند صفّ «الهدف» — وهو ينقلب
    # على نفسه متى كان الهدف أدنى من قمم الجوار (الفضة): يصير الحدُّ
    # أكبرَ فيدفع الوسم إلى داخل الأجساد. الحدُّ الصحيح هو حافة اللوحة.
    yy = hi - 26
    # وإن صادف صفَّ وسم «الهدف» (الذهب) فالمخرج **إلى أعلى** لا إلى أسفل:
    # أسفلُه الشموع، وأعلاه فراغ.
    if abs(yy - y(PLAN["TGT"])) < 40:
        yy = y(PLAN["TGT"]) - 46
    return max(PT + 30, yy)
...
if __name__ == "__main__":
    ...
    for i, L in enumerate(LAY, 1):
        pass
    reason_pool.register(TAG, [k for k, _t, _d in REASONS])
