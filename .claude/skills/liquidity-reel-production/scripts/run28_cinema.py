...
# 🔒 النقطة الفاصلة تجلس على ارتفاع الرقم الهندي، فـ«· ٣ دقائق»
# تُقرأ «٣٠ دقائق» — أي فريمٌ مضروبٌ في عشرة على شاشةٍ منشورة.
# الحلّ أن تسبق الرقمَ كلمةٌ فلا يلامس الفاصلَ رقمٌ أبداً.
def tf_ar(tf):
...
def _fit_pad(lo=1.45, hi=6.5):
    ...
    والسقف ٦٫٠ لا ٣٫٢ (2026-08-11): نافذة الخام 2026-08-11 04:18 وقفُها
    **هو قاع النافذة نفسه** (81.91)، فصفوف الوسوم الثلاثة تبدأ من أدنى
    شمعة ولا تجد تحتها بكسلاً. وحاجتُها ثابتة بالبكسل (٣٤ ثم ٤٠ ثم ٥٢)
    بينما `plo` يشتريها كنسبة من اللوحة — فالسقف القديم كان يمنع الحلّ
    لا يكشف عيباً: عند ٥٫٠ تتّسع الصفوف والشموع تملأ ٦٨٪ من اللوحة.
    وهذا **ليس تخفيض شرط**: الطبقات الخمس وحرّاس المواضع كما هي، والذي
    اتّسع مدى بحثِ الحلّ لا عتبةُ قبولٍ.

    والسقف بقي ٦٫٠ بعد تجربةٍ مقيسة (2026-08-12) — تُسجَّل كي لا تُعاد:
    نافذة الغاز 2026-08-10 12:00 مقبولةٌ في الطبقات الخمس وغيرُ منشورة،
    لكن المحرّك رفض رسمها. رُفع السقف إلى ١٢ فوجد الحلّ عند ٩٫٤٥ وبُني
    الريل ورُندر وفُحصت إطاراتُه — **فسقط بصرياً**:

      · مدى النافذة 0.037 فقط، والقيعان المتساوية 2.757 والدخول 2.758
        تفصلهما تكّةٌ واحدة ≈ بكسلان — فيُرسمان **خطاً واحداً**، والدرس
        كلُّه أن تُرى المسافة بينهما.
      · حصّة الشموع تنزل إلى ٥٤٫٥٪ (مقابل ٧٢٪ على نافذة الإيثيريوم
        المنشورة) ويبقى ربعُ اللوحة فراغاً تحت صفوف الوسوم.
      · شارات محور السعر تتراكب: 2.761 على 2.760، و2.752 على 2.750.

    والأول ليس عيبَ مواضعَ يُصلَح بإزاحة — هو خاصّةُ النافذة نفسها. فرُدّت
    ولم يُبنَ عليها. والقاعدة المستخلصة: **رفضُ الاتّساع هنا لم يكن سقفاً
    ضيّقاً بل كاشفاً عن نافذةٍ لا تحتمل مفرداتِ الماركب**؛ ومن أراد رفعه
    مستقبلاً فليقِسْ أولاً `(الدخول − مستوى السيولة) ÷ مدى النافذة`، فإن
    نزلت عن ~٥٪ فالخطّان سيتّحدان مهما اتّسع الهامش.

    ورُفع السقف ٦٫٠ ← ٦٫٥ (2026-09-14) بعد قياسٍ لا تخميناً: نافذة البتكوين
    2026-09-14 09:24 تطلب ٦٫١٥ — خمسَ عشرةَ جزءاً من مئةٍ فوق السقف وحدها.
    وقياسُ الفصل الذي وصفته الفقرةُ أعلاه يُعطيها ٥٫٨٨٪ فهي فوق الـ~٥٪، أي
    ليست من جنس نافذة الغاز. وحصّةُ الشموع عندها ٦٣٫٦٪ مقابل ٦٤٫١٪ عند
    السقف نفسه — فرقُ نصفِ نقطةٍ لا يقلب حكماً. والسقفُ الجديد ٦٫٥ لا ١٢:
    يكفي هذه النافذة ويبقى بعيداً عن المدى الذي أسقط الغاز بصرياً.
    والحكم النهائي للإطارات لا للحساب: بُنيت ورُندرت وفُحص إطاراها.
...
def m5_svg():
    ...
    # وسم «الوقف» يجلس فوق حافة الصندوق السفلى بمقدار حجمه، فيشغل شريطاً
    # ارتفاعه نحو ١٫٨ من الحجم فوق سعر الوقف. وخطُّ «قيعان متساوية» يمرّ
    # على سعره هو — وإن تقارب السعران عبَر الخطُّ الوسمَ في منتصفه فشطب
    # الرقم (برنت 2026-09-02: الوقف 94.60 والمستوى 94.62 — سنتان). فيُقاس
    # الشريط ويُسأل: أيقع خطُّ المستوى داخله؟ فإن وقع نُقل الوسم تحت
    # الحافة حيث لا خطّ.
    S_FS = 28
    ...
    # موضع ريبلاي، والصندوق يظهر عند ٢٢٫٤٠ ويبقى إلى آخر الريل.
    def _s_hits(p):
        a = mapy(y(PLAN["STP"]), p)
    ...
    S_CLASH = S_BELOW and not (S_CX + S_HW < LVL_X - LVL_W or S_CX - S_HW > LVL_X)
    ...
    YL_ROW = max(y(lvl), y(PLAN["STP"]), S_LBL_Y + 10 if S_CLASH else 0) + 34
    ...
    # والفحص على **كل لحظة** لا على المقياس الأساس وحده: المحور يتنفّس،
    # فصفٌّ يسع في إطارٍ يُقتصّ في إطارٍ بعده. وقع على برنت 2026-09-10:
    # الوقف 106.50 قريبٌ من قاع النافذة، فنزل صفُّ «سحب سيولة» تحت حافّة
    # اللوحة وقُصّ نصفُه (رُصد بالفحص البصري لإطار ٢٧٫٥ قبل الرندر).
    # فتُرفع الصفوف الثلاثة معاً بالفائض الأسوأ، حفاظاً على تباعدها.
    # والفائض يُؤخذ من الفجوتين أوّلاً لا برفع الصفوف الثلاثة: فوق الصفّ
    # الأعلى يجلس وسمُ الوقف، ورفعُه إليه يلصق الرقمين. والحدّ الأدنى بين
    # صفّين ٣٤ بكسل (نفس حدّ الفقرة أعلاه)، فما زاد عن ذلك يُرفع كتلةً
    # واحدة مع حارسٍ يمنع ملامسة وسم الوقف.
    _RP0 = int(_rp_p(18.10))
    _G1, _G2 = Y_WICK - YL_ROW, Y_SWP - Y_WICK

    def _swp_over(l, g1, g2):
        yy = YL_ROW - l + g1 + g2
        return max((mapy(yy, p) + 14) - (CVH - PB) for p in range(_RP0, len(M5)))

    _OVER = _swp_over(0, _G1, _G2)
    if _OVER > 0:
        _slack = (_G1 - 34) + (_G2 - 34)
        _take = min(_OVER, _slack)
        if _slack > 0:
            _G1 -= (_G1 - 34) * _take / _slack
            _G2 -= (_G2 - 34) * _take / _slack
        _LIFT = max(0.0, _swp_over(0, _G1, _G2))
        YL_ROW -= _LIFT
        Y_WICK = YL_ROW + _G1
        Y_SWP = Y_WICK + _G2
    # وسمُ الوقف في وضعه الافتراضي يجلس فوق حافّة الصندوق السفلى مباشرة.
    _S_DEF = y(PLAN["STP"]) - S_FS
    assert S_BELOW or YL_ROW - _S_DEF >= 34, \
        f'صفُّ القيعان لاصقَ وسمَ الوقف بعد الرفع ({YL_ROW - _S_DEF:.0f}px)'
    assert all(mapy(Y_SWP, p) + 14 <= CVH - PB for p in range(_RP0, len(M5))), \
        f'وسم «سحب سيولة» خارج اللوحة ({Y_SWP:.0f})'
    assert not S_BELOW or S_LBL_Y + 8 <= CVH - PB, f'وسم «الوقف» المنقول خارج اللوحة ({S_LBL_Y:.0f})'
