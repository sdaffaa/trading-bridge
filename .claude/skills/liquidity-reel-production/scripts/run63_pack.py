# -*- coding: utf-8 -*-
"""تشغيلة ٦٣ — الرندر والتغليف: صور الكاروسيل ١٠٨٠×١٣٥٠ وأدلة PDF بلا فقد.
...
وحدةٌ واحدة اليوم ريل (لا اثنتان): لها دليلٌ بلا كاروسيل — فالتغليف
يقرأ صيغة كل وحدة من ملفّها بدل أن يفترضها.

    python3 run63_pack.py [slug ...]
"""
...
OUT = os.path.join(HERE, "out63")


def units():
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if fn.startswith("run63_") and fn.endswith(".json"):
...
def main(slugs=None):
    ...
    for slug in (slugs or list(U)):
        ...
        if C.get("media") != "reel":
            ...
            car_render.render(os.path.join(HERE, f"car63_{slug}.html"), d)
        ...
        car_render.render(os.path.join(HERE, f"guide63_{slug}.html"), g)
