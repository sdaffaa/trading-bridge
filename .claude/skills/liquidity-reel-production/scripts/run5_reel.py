...
cfg["cam"] = [
  ...
  [2.9, 1.7, fxn(IB), fyn(BOSv), "anticip"],            # زوم الكسر — بترقّب
  [5.2, 1.85, fxn((IZ0+IE)/2), fyn((ZT+ZB)/2), "ramp"],  # منطقة الاهتمام — سبيد رامب
  [7.5, 1.5, fxn(IE), fyn(SL), "creep"],                 # الستوب والأهداف — زحف
  [9.7, 1.9, fxn(IE), fyn(ZB), "anticip"],               # الفوت برنت
  [11.9, 1.95, fxn(IE), fyn(W_[IE]["l"]), "ramp"],       # الدخلة
  [13.7, 1.95, fxn(IE), fyn(W_[IE]["l"])],               # فريز فريم لحظة الإمباكت (الماستر §9)
  [14.3, 1.65, fxn(ISEC), fyn(W_[IE]["l"]), "whip", -1.2],  # التأمين — ويب + دَتش
  [16.2, 1.3, fxn(32), fyn(W_[32]["c"]), "creep", 0],
