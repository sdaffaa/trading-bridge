# -*- coding: utf-8 -*-
"""ريلا تشغيلة ٥٤ — محرّك «جلسة متداول» على ملفّات محتوى ٥١.
...
كي لا يتشابه ريلان في يومٍ واحد.
    ...
    python3 run54_reel.py            # يبني الاثنين
    python3 run54_reel.py <slug>
"""
import json, os, sys
...
def load(slug):
    with open(os.path.join(CONT, f"run54_{slug}.json"), encoding="utf-8") as f:
        C = json.load(f)
...
def main(slugs=None):
    ...
    for slug in (slugs or list(plan)):
        ...
        # يقيّدها بنفسه. القيد واحدٌ للوحدة كلها في `run54_factory` —
