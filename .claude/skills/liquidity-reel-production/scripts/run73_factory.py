# -*- coding: utf-8 -*-
"""تشغيلة ٧٣ — مصنع المحتوى اليومي (§11): ثلاثةُ كاروسيلاتٍ وريلان وخمسةُ أدلة.

مفاتيحُها `content/run73_*.json`، وقيودُها بوسم `run73-<الوحدة>`،
ومخرجاتُها `car73_*` و`guide73_*`، والفرزُ يُقرأ من `X73.SYN` لا من
`X73.SETS`.

ووحداتُ اليوم صنفان: اثنتان على سوقٍ حقيقي (`register_real`) وثلاثٌ
حسابٌ محضٌ بافتراضاتٍ معلنة تُبصَم برقمٍ حرٍّ (`register_synthetic`).
والمزيجُ أربعةُ كاروسيلاتٍ وريلٌ واحد: بوابةُ `run32_desk.preflight` لا
تُثبت نمطاً على أيِّ نافذةٍ حرّةٍ غير ٧٨، والمزيجُ يُنقَص ولا يُفبرَك.

    python3 run73_factory.py [slug ...]
"""
import json, os, sys
...
import run73_charts as X73

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run73-factory-2026-09-18"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run73_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run73_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run73_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car73_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide73_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run73-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run73_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X73.SETS:
        if slug in X73.REAL:
            assert X73.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X73.unit_charts(slug)
...
def _claim(slug, r):
    ...
    ووحداتُ اليوم الثلاثُ الحسابيةُ لها بصمةٌ في السجل: رقمٌ حرٌّ
    ومرتكزٌ هو افتراضاتُها المعلنة، فلو أُعيدت الافتراضاتُ نفسُها كُشف
    التكرار."""
    if slug in X73.SYN:
        seed, anch = X73.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run73-{slug}")
    else:
        RC.claim_fresh(r, f"run73-{slug}")


def _register(slug, r):
    if slug in X73.SYN:
        seed, anch = X73.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run73-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run73-{slug}")
