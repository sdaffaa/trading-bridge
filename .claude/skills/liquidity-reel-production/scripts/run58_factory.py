# -*- coding: utf-8 -*-
"""تشغيلة ٥٨ — مصنع المحتوى اليومي (§11): ثلاثة كاروسيلات وثلاثة أدلة.
...
**الوحدتان الفنيّتان مؤجَّلتان**: التوزيع المطلوب ٢ فنية + ١ نفسية + ١
أساسية + ١ مالية، والفنيّتان تُسلَّمان ريلين — والريل يمرّ ببوابة
`run32_desk.py --preflight`. والمخزون اليوم تسع نوافذ حرّة، **لا واحدة
منها تجتاز** البوابة (ثلاثة أسباب + هدف ٢R). فبقيت الوحدات الثلاث
المستقلّة عن نافذةِ ريل، وتأجّلت الفنيّتان لرأس الطابور بلا تخفيض عتبة.

كل نافذة يومية حقيقية حرّة من `sheet_candidates.json`، وتُفحص حرّيتها بالسجل قبل البناء وتُقيَّد بعده.

    python3 run58_factory.py [slug ...]
"""
import json, os, sys
...
import run58_charts as X58

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run58-factory-2026-08-12"
TOTAL = 3
TEAL_D = "#1E627A"
...
def _units():
    """يقرأ ملفات `content/run58_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run58_") and fn.endswith(".json")):
            continue
        slug = fn[len("run58_"):-len(".json")]
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car58_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide58_{slug}.html")
...
def build(slug, C):
    # حالات `run31_charts` السبع نماذجُ دخول، ووحدات هذه التشغيلة ليست
    # درسَ دخول — فجارتاتها من `run58_charts`: قياسٌ لا صفقة.
    r, ok, dropped = X58.unit_charts(slug)
    assert X58.WIN[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
    RC.claim_fresh(r, f"run58-{slug}")
    ns = 0
    if C.get("media") != "reel":
        _, ns = build_car(slug, C, r, ok)
    gpath, ng, tk = build_gd(slug, C, r, ok)
    RC.register(RUN_ID, r, f"run58-{slug}")
