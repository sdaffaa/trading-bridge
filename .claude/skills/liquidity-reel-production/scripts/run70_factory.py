# -*- coding: utf-8 -*-
"""تشغيلة ٧٠ — مصنع المحتوى اليومي (§11): ثلاثة كاروسيلات وريلان وخمسة أدلة.

مفاتيحُها `content/run70_*.json`، وقيودُها بوسم `run70-<الوحدة>`،
ومخرجاتُها `car70_*` و`guide70_*`، والفرزُ يُقرأ من `X70.SYN` لا من
`X70.SETS`.

والمخزون أعطى **نافذةً واحدة حرّة** من اثنتين وسبعين (قِيس الفراغُ
بالدالّة التي تحرسه: `run31_charts._span` ثم `assert_fresh_real`) — هي
الكاردانو/دولار · ساعة · 2025-10-03، وكانت قد سُجّلت في تشغيلة ٦٩ ثم
أُلغي قيدُها حين أُسقطت وحدةُ «ترند» لخروجها عن النطاق. فوحدةٌ فنيةٌ
واحدة على سوقٍ حقيقي (`register_real`) وأربعٌ سلاسلُ مولّدة
(`register_synthetic`) بقاعدة §11 البديلة.

    python3 run70_factory.py [slug ...]
"""
import json, os, sys
...
import run70_charts as X70

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run70-factory-2026-09-15"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run70_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run70_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run70_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car70_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide70_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run70-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run70_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X70.SETS:
        if slug in X70.REAL:
            assert X70.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X70.unit_charts(slug)
...
def _claim(slug, r):
    ...
    if slug in X70.SYN:
        seed, anch = X70.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run70-{slug}")
    else:
        RC.claim_fresh(r, f"run70-{slug}")


def _register(slug, r):
    if slug in X70.SYN:
        seed, anch = X70.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run70-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run70-{slug}")
