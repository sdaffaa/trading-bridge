# -*- coding: utf-8 -*-
"""تشغيلة ٦١ — مصنع المحتوى اليومي (§11): ثلاثة كاروسيلات وريلان وخمسة أدلة.

نسخةُ عملٍ من `run61_factory` بمفاتيحها هي: `content/run61_*.json`،
وقيودُها بوسم `run61-<الوحدة>`، ومخرجاتُها `car61_*` و`guide61_*`.

**الجارت يتبع درسَ الوحدة لا محرّكها** (درس ٥٨): الوحدتان الفنيّتان
ريلان يبنيهما `run61_reels.py` على `run32_desk`، ولكلٍّ هنا **دليلُه**
وحده. والوحدات الثلاث الأخرى دروسُ قياس فجارتاتُها من `run61_charts`.

    python3 run61_factory.py [slug ...]
"""
import json, os, sys
...
import run61_charts as X61

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run61-factory-2026-09-06"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run61_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        ...
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run61_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car61_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide61_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run61-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    ...
    if slug in X61.SETS:
        r, ok, dropped = X61.unit_charts(slug)
        assert X61.WIN[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return r, ok, dropped
...
def build(slug, C):
    ...
    RC.claim_fresh(r, f"run61-{slug}")
    ns = 0
    if C.get("media") != "reel":
        _, ns = build_car(slug, C, r, ok)
    gpath, ng, tk = build_gd(slug, C, r, ok)
    RC.register(RUN_ID, r, f"run61-{slug}")
