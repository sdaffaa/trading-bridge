# -*- coding: utf-8 -*-
"""تشغيلة ٦٤ — الرندر والتغليف: صور الكاروسيل ١٠٨٠×١٣٥٠ وأدلة PDF بلا فقد.
...
وحدات اليوم كلُّها كاروسيلات: مخزون النوافذ الحقيقية نفد على الفريمين
معاً، ولا يُبنى ريلُ صفقةٍ على سلسلةٍ مولّدة (يطبع دخولاً ووقفاً وهدفاً
لا وجود لها). والتغليف يقرأ صيغة كل وحدة من ملفّها بدل أن يفترضها.

    python3 run64_pack.py [slug ...]
"""
...
OUT = os.path.join(HERE, "out64")


def units():
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if fn.startswith("run64_") and fn.endswith(".json"):
...
def main(slugs=None):
    ...
    for slug in (slugs or list(U)):
        ...
        if C.get("media") != "reel":
            ...
            car_render.render(os.path.join(HERE, f"car64_{slug}.html"), d)
        ...
        car_render.render(os.path.join(HERE, f"guide64_{slug}.html"), g)
