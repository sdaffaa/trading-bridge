# -*- coding: utf-8 -*-
"""تشغيلة ٦٣ — مصنع المحتوى اليومي (§11): أربعة كاروسيلات وريلٌ وخمسة أدلة.

نسخةُ عملٍ من `run61_factory` بمفاتيحها هي: `content/run63_*.json`،
وقيودُها بوسم `run63-<الوحدة>`، ومخرجاتُها `car63_*` و`guide63_*`.

**والفرق الجوهري عن ٦١**: مخزون النوافذ الحقيقية نفد إلا نافذتين، فأخذتهما
الوحدتان الفنيّتان، والثلاث الباقية تخطيطية بقاعدة §11 البديلة. ولذلك
انقسم القيد قسمين — النوافذ الحقيقية تُقيَّد بـ`register_real`، والسلاسل
التخطيطية بـ`register_synthetic` ببصمة (بذرة، مراسٍ) — وإلا مرّت السلسلة
نفسها في تشغيلةٍ قادمة كأنها جديدة.

    python3 run63_factory.py [slug ...]
"""
import json, os, sys
...
import run63_charts as X63

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run63-factory-2026-09-07"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run63_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run63_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run63_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car63_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide63_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run63-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run63_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X63.SETS:
        if slug in X63.REAL:
            assert X63.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X63.unit_charts(slug)
...
def _claim(slug, r):
    ...
    if slug in X63.SYN:
        seed, anch = X63.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run63-{slug}")
    else:
        RC.claim_fresh(r, f"run63-{slug}")


def _register(slug, r):
    if slug in X63.SYN:
        seed, anch = X63.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run63-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run63-{slug}")
