# -*- coding: utf-8 -*-
"""تشغيلة ٦٤ — مصنع المحتوى اليومي (§11): ثلاثة كاروسيلات وثلاثة أدلة.

نسخةُ عملٍ من `run61_factory` بمفاتيحها هي: `content/run64_*.json`،
وقيودُها بوسم `run64-<الوحدة>`، ومخرجاتُها `car64_*` و`guide64_*`.

**والفرق الجوهري عن ٦٣**: مخزون النوافذ الحقيقية نفد على الفريمين معاً
(صفرٌ حرّ من سبعٍ وستّين مرشّحة، ومسحُ الفريم اليومي أعطى ٣٣ مرشّحاً
كلَّها ساقطة)، فوحدات اليوم الثلاث كلُّها سلاسل تخطيطية بقاعدة §11
البديلة. وقيدُها كلِّه `register_synthetic` ببصمة (بذرة، مراسٍ) — وإلا
مرّت السلسلة نفسها في تشغيلةٍ قادمة كأنها جديدة. ولا ريل اليوم: ريلُ
الصفقة يطبع دخولاً ووقفاً وهدفاً، وطبعُها على سلسلةٍ مولّدة كذب.

    python3 run64_factory.py [slug ...]
"""
import json, os, sys
...
import run64_charts as X64

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run64-factory-2026-09-09"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run64_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run64_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run64_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car64_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide64_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run64-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run64_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X64.SETS:
        if slug in X64.REAL:
            assert X64.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X64.unit_charts(slug)
...
def _claim(slug, r):
    ...
    if slug in X64.SYN:
        seed, anch = X64.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run64-{slug}")
    else:
        RC.claim_fresh(r, f"run64-{slug}")


def _register(slug, r):
    if slug in X64.SYN:
        seed, anch = X64.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run64-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run64-{slug}")
