# -*- coding: utf-8 -*-
"""تشغيلة ٥٩ — الرندر والتغليف: صور الكاروسيل ١٠٨٠×١٣٥٠ وأدلة PDF بلا فقد.
...
OUT = os.path.join(HERE, "out59")


def units():
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if fn.startswith("run59_") and fn.endswith(".json"):
            with open(os.path.join(CONT, fn), encoding="utf-8") as f:
                out[fn[6:-5]] = json.load(f)
    return out
...
def main(slugs=None):
    ...
    for slug in (slugs or list(U)):
        ...
        if C.get("media") != "reel":
            ...
            car_render.render(os.path.join(HERE, f"car59_{slug}.html"), d)
        ...
        car_render.render(os.path.join(HERE, f"guide59_{slug}.html"), g)
