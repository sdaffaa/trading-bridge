# -*- coding: utf-8 -*-
"""تشغيلة ٦٦ — مصنع المحتوى اليومي (§11): ثلاثة كاروسيلات وخمسة أدلة وريلان.

نسخةُ عملٍ من `run65_factory` بمفاتيحها هي: `content/run66_*.json`، وقيودُها
بوسم `run66-<الوحدة>`، ومخرجاتُها `car66_*` و`guide66_*`.

**والفرق الجوهري عن ٦٤ و٦٥**: المخزون لم يعد صفراً. تقاطعُ «سليمة في
`fix_candidates`» مع «حرّة في السجل» أعطى **خمس** نوافذ — فالوحدتان
الفنيتان على سوقٍ حقيقي (`register_real`) والثلاث الباقية سلاسلُ مولّدة
(`register_synthetic`). والفرزُ يُقرأ من `X66.SYN` لا من `X66.SETS`.

    python3 run66_factory.py [slug ...]
"""
import json, os, sys
...
RUN_ID = "run66-factory-2026-09-11"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run66_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run66_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run66_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car66_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide66_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run66-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run66_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X66.SETS:
        if slug in X66.REAL:
            assert X66.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X66.unit_charts(slug)
...
def _claim(slug, r):
    ...
    if slug in X66.SYN:
        seed, anch = X66.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run66-{slug}")
    else:
        RC.claim_fresh(r, f"run66-{slug}")


def _register(slug, r):
    if slug in X66.SYN:
        seed, anch = X66.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run66-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run66-{slug}")
