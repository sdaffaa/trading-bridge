# -*- coding: utf-8 -*-
"""تشغيلة ٦٥ — مصنع المحتوى اليومي (§11): خمسة كاروسيلات وخمسة أدلة.

نسخةُ عملٍ من `run61_factory` بمفاتيحها هي: `content/run65_*.json`،
وقيودُها بوسم `run65-<الوحدة>`، ومخرجاتُها `car65_*` و`guide65_*`.

**والفرق الجوهري عن ٦٤**: مخزون النوافذ الحقيقية ما زال صفراً حرّاً على
الفريمين (يومٌ ثانٍ)، فوحدات اليوم الخمس كلُّها سلاسل تخطيطية بقاعدة §11
البديلة، وقيدُها كلِّه `register_synthetic` ببصمة (بذرة، مراسٍ) — وإلا
مرّت السلسلة نفسها في تشغيلةٍ قادمة كأنها جديدة. ولا ريل اليوم كذلك:
ريلُ الصفقة يطبع دخولاً ووقفاً وهدفاً، وطبعُها على سلسلةٍ مولّدة كذب.

    python3 run65_factory.py [slug ...]
"""
import json, os, sys
...
import run65_charts as X65

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run65-factory-2026-09-10"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run65_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run65_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run65_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car65_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide65_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run65-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run65_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X65.SETS:
        if slug in X65.REAL:
            assert X65.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X65.unit_charts(slug)
...
def _claim(slug, r):
    ...
    if slug in X65.SYN:
        seed, anch = X65.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run65-{slug}")
    else:
        RC.claim_fresh(r, f"run65-{slug}")


def _register(slug, r):
    if slug in X65.SYN:
        seed, anch = X65.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run65-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run65-{slug}")
