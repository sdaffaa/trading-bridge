...
G_KASR = dict(
  ...
  subtitle="الفتيل يكذب… الجسم يصدق", hero=dkmap(kasr_static(760, 360)),
  pages=[
    dict(title="نفس المستوى… كسرين",
         lead="السعر يوصل المستوى مرتين: مرة يخترقه بالفتيل ويرجع، ومرة يسكّر فوقه بجسم كامل. الأول ياخذ ستوبات… والثاني يبدأ حركة.",
         svg=kasr_static()),
    dict(title="علامات الكسر الوهمي",
         rules=[dict(t="فتيل يخترق — والإغلاق يرجع تحت المستوى", bad=True),
                dict(t="بدون اندفاع — شمعة ضعيفة الجسم", bad=True),
                dict(t="يجي بعد حركة ممتدة — آخر نفس، مو بداية", bad=True)]),
    dict(title="علامات الكسر الحقيقي",
         rules=[dict(t="جسم كامل يسكّر فوق المستوى"),
                dict(t="اندفاع واضح — شمعة كبيرة بحجم"),
                dict(t="يجي بعد تجميع/رجعة — مو بعد تعب")]),
    dict(title="شلون تتعامل؟",
         rules=[dict(t="لا تدخل مع أول اختراق — انتظر الإغلاق"),
                dict(t="انطق ستوبك بفيك؟ لا تطارد — انتظر الرجعة"),
                dict(t="الكسر الحقيقي يعطيك دخلة ثانية عالريتيست")],
         note="الإغلاق اهو الحكم — مو الفتيل."),
  ],
  outro_title="الجسم يصدق", outro_items=[
    "قبل ما تقول «انكسر» — شوف وين سكّرت الشمعة",
    "الفيك مو عدوك… اهو إشارة إن السيولة انسحبت",
    "بعد الفيك، الكسر الحقيقي يجي أقوى"])

G_SUYULA = dict(
  eyebrow="دليل — السيولة", title="أنواع تجمعات<br>السيولة", keyword="سيولة",
  subtitle="السعر يمشي من سيولة… لسيولة", hero=dkmap(suyula_static(760, 360)),
  pages=[
    dict(title="وين تتجمع الستوبات؟",
         rules=[dict(t="فوق القمم المتساوية — ستوبات البايعين"),
                dict(t="تحت القيعان المتساوية — ستوبات الشارين"),
                dict(t="على الترند لاين الواضح — كل اللي راسمينه")],
         note="كل ما كان المستوى أوضح… السيولة فوقه أكثر."),
    dict(title="مثال من السوق الحقيقي", ticker="الناسداك · 30 دقيقة · 2026-06-18",
         lead="قمم متساوية فوق وPDL تحت — السوق نزل خذ سيولة القاع، ارتد من الزون، وطلع خذ سيولة القمم.",
         svg=suyula_static()),
    dict(title="شلون تستفيد؟",
         rules=[dict(t="حدد تجمعات السيولة قبل الجلسة"),
                dict(t="توقع السحب — لا تدخل قبله"),
                dict(t="ادخل بعد السحب مع الارتداد من الزون")]),
    dict(title="أخطاء شائعة",
         rules=[dict(t="تحط ستوبك مع القطيع — فوق نفس القمم", bad=True),
                dict(t="تشوف السحب وتدخل عكسه بدون تأكيد", bad=True),
                dict(t="تنسى إن السيولة هدف — مو مقاومة", bad=True)]),
  ],
  outro_title="فكر مثل السوق", outro_items=[
    "اسأل دايم: وين الستوبات متجمعة الحين؟",
    "السحب مو مصادفة — اهو وقود الحركة",
    "خل ستوبك بعيد عن التجمعات الواضحة"])
