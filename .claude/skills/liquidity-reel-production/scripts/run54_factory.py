# -*- coding: utf-8 -*-
"""تشغيلة ٥٤ — مصنع المحتوى اليومي (§11): أربعة كاروسيلات وخمسة أدلة.
...
والريل يُبنى في `run32_desk.py` على نافذته، وله هنا **دليله**
وحده — فالوحدة الواحدة تصميمٌ ودليلٌ معاً أياً كانت صيغة التصميم.

كل نافذة يومية حقيقية حرّة من `sheet_candidates.json`، وتُفحص حرّيتها بالسجل قبل البناء وتُقيَّد بعده.

    python3 run54_factory.py [slug ...]
"""
import json, os, sys
...
RUN_ID = "run54-factory-2026-08-11"
TOTAL = 3
TEAL_D = "#1E627A"
...
def _units():
    """يقرأ ملفات `content/run54_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run54_") and fn.endswith(".json")):
            continue
        slug = fn[len("run54_"):-len(".json")]
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car54_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide54_{slug}.html")
...
def build(slug, C):
    ...
    RC.claim_fresh(r, f"run54-{slug}")
    ns = 0
    if C.get("media") != "reel":
        _, ns = build_car(slug, C, r, ok)
    gpath, ng, tk = build_gd(slug, C, r, ok)
    RC.register(RUN_ID, r, f"run54-{slug}")
