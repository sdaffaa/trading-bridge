# -*- coding: utf-8 -*-
"""تشغيلة ٧٠ — الرندر والتغليف: صور الكاروسيل ١٠٨٠×١٣٥٠ وأدلة PDF بلا فقد.
...
OUT = os.path.join(HERE, "out70")


def units():
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if fn.startswith("run70_") and fn.endswith(".json"):
...
def main(slugs=None):
    ...
    for slug in (slugs or list(U)):
        ...
        if C.get("media") != "reel":
            ...
            car_render.render(os.path.join(HERE, f"car70_{slug}.html"), d)
        ...
        car_render.render(os.path.join(HERE, f"guide70_{slug}.html"), g)
