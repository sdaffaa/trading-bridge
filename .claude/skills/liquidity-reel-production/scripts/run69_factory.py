# -*- coding: utf-8 -*-
"""تشغيلة ٦٩ — مصنع المحتوى اليومي (§11): ثلاثة كاروسيلات وريلان وخمسة أدلة.

مفاتيحُها `content/run69_*.json`، وقيودُها بوسم `run69-<الوحدة>`،
ومخرجاتُها `car69_*` و`guide69_*`، والفرزُ يُقرأ من `X69.SYN` لا من
`X69.SETS`.
...
import run69_charts as X69

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run69-factory-2026-09-14"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run69_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if not (fn.startswith("run69_") and fn.endswith(".json")):
            continue
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run69_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car69_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide69_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run69-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """كلُّ وحدةٍ لها مجموعتُها في `run69_charts` — تخطيطيةً كانت أو سوقية.
    ...
    if slug in X69.SETS:
        if slug in X69.REAL:
            assert X69.REAL[slug] == C["window"], f"{slug}: نافذة الوحدة لا تطابق ملفها"
        return X69.unit_charts(slug)
...
def _claim(slug, r):
    ...
    if slug in X69.SYN:
        seed, anch = X69.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run69-{slug}")
    else:
        RC.claim_fresh(r, f"run69-{slug}")


def _register(slug, r):
    if slug in X69.SYN:
        seed, anch = X69.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run69-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run69-{slug}")
