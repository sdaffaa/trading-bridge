# -*- coding: utf-8 -*-
"""تشغيلة ٦٦ — الرندر والتغليف: صور الكاروسيل ١٠٨٠×١٣٥٠ وأدلة PDF بلا فقد.
...
والتغليف يقرأ صيغة كل وحدة من ملفّها بدل أن يفترضها: وحدةُ الريل لا
كاروسيل لها، ولها دليلُها كغيرها.

    python3 run66_pack.py [slug ...]
"""
...
OUT = os.path.join(HERE, "out66")


def units():
    out = {}
    for fn in sorted(os.listdir(CONT)):
        if fn.startswith("run66_") and fn.endswith(".json"):
...
def main(slugs=None):
    ...
    for slug in (slugs or list(U)):
        ...
        if C.get("media") != "reel":
            ...
            car_render.render(os.path.join(HERE, f"car66_{slug}.html"), d)
        ...
        car_render.render(os.path.join(HERE, f"guide66_{slug}.html"), g)
