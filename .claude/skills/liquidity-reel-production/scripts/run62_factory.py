# -*- coding: utf-8 -*-
"""تشغيلة ٦٢ — مصنع المحتوى اليومي (§11): أربعة كاروسيلات وريلٌ وخمسة أدلة.

نسخةُ عملٍ من `run61_factory` بمفاتيحها هي: `content/run62_*.json`،
وقيودُها بوسم `run62-<الوحدة>`، ومخرجاتُها `car62_*` و`guide62_*`.
...
import run62_charts as X62

HERE = os.path.dirname(os.path.abspath(__file__))
CONT = os.path.normpath(os.path.join(HERE, "..", "content"))
RUN_ID = "run62-factory-2026-09-07"
TOTAL = 3
TEAL_D = "#1E627A"


def _units():
    """يقرأ ملفات `content/run62_*.json` — الوحدة تُعلن نافذتها وصيغتها."""
    out = {}
    for fn in sorted(os.listdir(CONT)):
        ...
        with open(os.path.join(CONT, fn), encoding="utf-8") as f:
            out[fn[len("run62_"):-len(".json")]] = json.load(f)
    return out
...
def build_car(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"car62_{slug}.html")
...
def build_gd(slug, C, r, ok):
    ...
    out = os.path.join(HERE, f"guide62_{slug}.html")
...
def _unregister(slugs):
    ...
    drop = {f"run62-{s}" for s in slugs} | {f"run32-{s}" for s in slugs}
...
def charts_of(slug, C):
    """الوحدة التخطيطية سلسلتُها من `run62_charts`، والحقيقيةُ نافذةٌ بفهرس."""
    if slug in X62.SETS:
        return X62.unit_charts(slug)
    return RC.unit_charts(C["window"])


def _claim(slug, r):
    if slug in X62.SETS:
        seed, anch = X62.SYN[slug]
        chart_registry.assert_fresh_synthetic(seed, anch, label=f"run62-{slug}")
    else:
        RC.claim_fresh(r, f"run62-{slug}")


def _register(slug, r):
    if slug in X62.SETS:
        seed, anch = X62.SYN[slug]
        chart_registry.register_synthetic(RUN_ID, [(seed, anch, f"run62-{slug}")])
    else:
        RC.register(RUN_ID, r, f"run62-{slug}")
